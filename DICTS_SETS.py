...
def nestle(nestle_products : dict ) -> str : 
    '''This function receives a dict and returns top nestle product and nestle top sales figure'''
    ...
    key_list : list = list(nestle_products.keys())
    val_list : list = list(nestle_products.values())
 
    position : int = val_list.index(nestle_top_sales_figure)
    Top_nestle_product : str = key_list[position]

    print(f'\nTop selling product from Nestle is {Top_nestle_product} and sales figure is {nestle_top_sales_figure} US Dollar ..')

def unilever(unilever_products : dict ) -> str :
    '''This function receives a dict and returns top unilever product and unilever top sales figure'''
    ...
    unilever_top_sales_figure : int = 0
    ...
    key_list : list = list(unilever_products.keys())
    val_list : list = list(unilever_products.values())

    position : int = val_list.index(unilever_top_sales_figure)
    top_unilever_product : str = key_list[position]
...
number_of_nestle_products : int = len(list(nestle_products.keys()))
number_of_unilever_products : int = len(list(unilever_products.keys()))
...
nestle_cities : set = {"SaudiArabia", "Oman", "Kuwait", "Egypt", "Jordan", "Sudan"}
unilever_cities : set = {"SaudiArabia", "Kuwait", "Iraq", "Morocco", "Yemen", "United Emirates"}
